...
WHERE q.id = '$quiz_id';  -- Replace 13 with the actual quiz ID
";

$quiz_details_result = mysqli_query($connection, $quiz_details_query);

// Check if quiz details are found
if ($quiz_details_result && mysqli_num_rows($quiz_details_result) > 0) 
{
    $quiz_details = mysqli_fetch_assoc($quiz_details_result);
} 
else {
    echo "<div class='alert alert-warning'>No quiz details found for the provided Quiz ID.</div>";
